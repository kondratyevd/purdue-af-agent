...
def agent_llm(state: AgentState) -> AgentState:
    ...
    writer = get_stream_writer()
    writer(f"🤖 Invoking agent LLM (ctx={len(messages)} msgs)")
...
def tool_node(state: AgentState) -> AgentState:
    ...
    writer = get_stream_writer()
    writer(f"🔧 Executing {len(tool_calls)} tool call(s)")
    ...
    for tool_call in tool_calls:
        ...
        if tool_name not in TOOLS_BY_NAME:
            writer(f"  ⚠️  Skipping unknown tool: {tool_name}")
            observations.append(
            ...
            try:
                ...
                writer(f"  ✗ Error executing {tool_name}: {e}")
...
def think_node(state: AgentState) -> AgentState:
    ...
    writer = get_stream_writer()
    writer(f"🤔 Reflecting on tool execution: {last_tool_msg.name}")
...
def finalize_output(state: AgentState) -> AgentState:
    ...
    writer = get_stream_writer()
    writer(f"📝 Finalizing output (ctx={len(state['messages'])} msgs)")
    messages = state["messages"]
    ...
    writer(f"📊 Final: username={username}, start={start_time}, end={end_time}")
...
def send_rejection(state: AgentState) -> AgentState:
    ...
    writer = get_stream_writer()
    writer("❌ Sending rejection message for non-profiling query")
    return {
        "agent_summary": (
...
def routing_after_agent_decision(state: AgentState) -> Literal["tools", "finalize"]:
    ...
    if current_count >= settings.max_tool_iterations:
        writer(f"⛔ Max iterations ({settings.max_tool_iterations}) reached")
        return "finalize"

    if last_ai_msg and last_ai_msg.tool_calls:
        writer(f"➡️  tools (iter {current_count + 1}/{settings.max_tool_iterations})")
        return "tools"
